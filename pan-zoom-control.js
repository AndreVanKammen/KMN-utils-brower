  ...
  constructor(element, options) {
    ...
    // Pan control
    {
      ...
      this.element.onpointerdown = (event) => {
        ...
        mouseMoved = false;
        mouseDown = true;
        mouseDownIsOurs = !this.onDown(this.mouseX, this.mouseY);
        if (mouseDownIsOurs) {
          this.isPanning = true;
      ...
      this.element.onpointermove = (event) => {
        ...
        let deltaPx = Math.max(Math.abs(deltaX * this.element.clientWidth), Math.abs(deltaY * this.element.clientHeight));
        if (mouseDown && deltaPx > 2.0) {
          let clickTime = (performance.now() - mouseDownTime);
          if (clickTime > 200 || (deltaPx * 10 > clickTime)) {
            mouseMoved = true;
          }
        }
        if (mouseDownIsOurs && !this.haltDragging) {
